...
export class Player {
    ...
    level = 0
    lavaSet = [[100, 0, 0, 0, 0], [80, 100, 0, 0, 0], [60, 90, 100, 0, 0], [20, 60, 80, 95, 100]]
    ...
    gameTick(){
      ...
      if(this.damageTimer > this.damageTimeLimit){
        ...
        this.game.display.coinLog = `${this.lavaSet[this.level]} : ${randomVal} : ${this.lavaSet[this.level][1]}` 
        if(randomVal < this.lavaSet[this.level][0] && false) {
        ...
        if(this.lava[i].livingTime < 0){
          if(this.lava[i].type=="scatterL"){
            for(let g = 0; g < 5; g++){
              this.lava[this.lava.length] = new Coin(5000, this.game, "5", "#dfbf8d")
              this.lava[this.lava.length-1].spawn()
              this.lava[this.lava.length-1].velX = (Math.random()-.5)*20
              this.lava[this.lava.length-1].velY = (Math.random()-.5)*20
              this.lava[this.lava.length-1].X = this.lava[i].X
              this.lava[this.lava.length-1].Y = this.lava[i].Y
