...
export class Draw {
    ...
    prest = true
    xOffset = 150
    colors = ["#90b0c0", "#afbfaf", "#c0b090", "#bfafbf", "#bfcfaf", "#dfafaf", "#ffffff", "#00ffff"]
    colorIndex = 0
        ...
        if(this.game.Style){
            if(!this.game.player.ascend){
                ...
                for(let i = 0; i < this.game.player.lava.length; i++){
                    this.Circ(this.game.player.lava[i].R, "#df8f8d", this.game.player.lava[i].X, this.game.player.lava[i].Y, true, "#6f3333")
                }  
            }
