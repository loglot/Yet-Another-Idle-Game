...
export class Draw {
    ...
    prest = true
    xOffset = 150
    colors = ["#90b0c0", "#afbfaf", "#c0b090", "#bfafbf", "#bfcfaf", "#dfafaf", "#90b0c0"]
    colorIndex = 0  
    ...
    drawGame(){
        ...
        if(this.game.Style){
            ...
            for(let i = 0; i < this.game.player.coins.length; i++){
                this.Circ(this.game.player.coins[i].R, "#dfdf8d", this.game.player.coins[i].X, this.game.player.coins[i].Y, true, "#5f5f33")
            }  
            for(let i = 0; i < this.game.player.lava.length; i++){
                this.Circ(this.game.player.lava[i].R, "#df8f8d", this.game.player.lava[i].X, this.game.player.lava[i].Y, true, "#5f3333")
            }  
